...
IF (EXISTS (
      SELECT * 
			...
			WHERE TABLE_CATALOG='eshop' AND TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'KHACHHANG'
    ))
BEGIN
...
IF (EXISTS (
      SELECT * 
			FROM INFORMATION_SCHEMA.TABLES
			WHERE TABLE_CATALOG='eshop' AND TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'NHANVIEN'
    ))
BEGIN
...
IF (EXISTS (
      SELECT * 
			FROM INFORMATION_SCHEMA.TABLES
			WHERE TABLE_CATALOG='eshop' AND TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'SANPHAM'
    ))
BEGIN
...
IF (EXISTS (
      SELECT * 
			FROM INFORMATION_SCHEMA.TABLES
			WHERE TABLE_CATALOG='eshop' AND TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'HOADON'
    ))
BEGIN
...
IF (EXISTS (
      SELECT * 
			FROM INFORMATION_SCHEMA.TABLES
			WHERE TABLE_CATALOG='eshop' AND TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'CTHD'
    ))
BEGIN
	DROP TABLE [dbo].[CTHD]
END
...
BEGIN
  	CREATE TABLE [dbo].[CTHD](
		...
		[MASP]   [char](4),
		[SL] [int],
	CONSTRAINT [PK_SOHD_MASP] PRIMARY KEY CLUSTERED
	(
		[SOHD] ASC,
        [MASP] ASC
	)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
...
INSERT INTO SANPHAM(MASP,TENSP,DVT,NUOCSX,GIA) VALUES ('ST11','BUT LONG','CAY','TRUNGQUOC',7000)
...
INSERT INTO HOADON(SOHD,NGHD,MAKH,MANV,TRIGIA) VALUES (1024,'17/01/2007',NULL,'NV01',5200000)
